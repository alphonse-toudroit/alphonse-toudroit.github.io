            ...
            link: function(scope){
                    ...
                    for (var i = 0; i < data.laps.length; i++) {
                        ...
                        if (laps != undefined) {
                            for (var j = 0; j < laps.length; j++) {
                                ...
                                marker.lap = lap-1;
                                marker.placing = lapData.placing[lap-1];
                                marker.name = lapData.name;
                                markers[p++] = marker;
								
								console.log("************************");
								console.log("marker.start " + marker.start );
								console.log("marker.placing " + marker.placing );
                ...
                function visualize(data) {
                   ...
                   addLapTickLines(vis, data.lapCount);
                    ...
                    addDriverLabels(vis, data.laps, 'flag', SCALES.x(data.lapCount-1) + PADDING.left + 10, 'start') // car un tour d'écart
                        .attr('y', function (d, i) {
                            return SCALES.y(i);
                        });
                ...
                function configureScales(data) {
                    SCALES.x = d3.scale.linear()
                        .domain([-1, data.lapCount-1])  // -1 car pas la position sur la grille
                ...
                //
                function highlight(vis, name, start, type) {
                    // Dim others.
                    vis.selectAll('polyline')
                        .style('opacity', function(d) {
                            return (d.name == name && d.placing[0] == start) ? HIGHLIGHT_OPACITY : DIMMED_OPACITY;
                        });

                    vis.selectAll('circle')
                        .style('opacity', function(d) {
                            return (d.name == name && d.start == start) ? HIGHLIGHT_OPACITY : DIMMED_OPACITY;
                        });
                    ...
                    vis.selectAll('text.label')
                        .style('opacity', function(d) {
							...
							//if (type == "placing") {							
								return (d.name == name && (d.placing[0] == start || d.start == start)) ? HIGHLIGHT_OPACITY : DIMMED_OPACITY;
							//}
							//else return (d.name == name && d.start == start) ? HIGHLIGHT_OPACITY : DIMMED_OPACITY;
                        });
                }
                ...
                function addLapTickLines(vis, lapCount) {
                    vis.selectAll('line.tickLine')
                        ...
                        .attr('visibility', function(d) {
                            return (d >= 0 && d <= lapCount) ? 'visible' : 'hidden'
                        });
                }
                ...
                function addPlacingsLines(vis, laps) {
                    vis.selectAll('polyline.placing')
                        ...
                        .style('stroke', function(d) {
                            return SCALES.clr(d.placing[0], "placing");
                        })
                        .on('mouseover', function(d) {
                            highlight(vis, d.name, d.placing[0], "placing");
                        })
                ...
                function addMarkers(vis, data, cssClass, label) {
                    ...
                    vis.selectAll("circle.marker." + cssClass)
                        ...
                        .on('mouseover', function(d) {
                            highlight(vis, d.name, d.start, "start");
                        })
                        .on('mouseout', function() {
                            unhighlight(vis);
                        });
